...
### 2026-09-10 — Lite 설치파일을 GitHub Release로 공개 배포 + API 키 번들링 기본값을 OFF로 변경
- 상황: "00. 배포용의 Lite 설치파일을 GitHub에 올려서 다른 환경(웹 버전 Claude Code 포함)에서
  받을 수 있냐"고 질문받음. 확인해보니 저장소(`leeb8901-design/datasheet-auto-analysis-
  program_260721`)가 **퍼블릭**인데, 그 설치파일에는 사용자의 실제 Mouser API 키가 번들링돼
  있어서(2026-09-04/09-09 결정 로그 참고) 그대로 올리면 키가 인터넷에 공개되는 상황이었음.
- 결정: (a) 이 사실을 먼저 명확히 알리고, 사용자가 "키 없는 버전으로 진행"을 선택함(확인 질문
  후 진행). (b) 재발 방지를 위해 **API 키 번들링을 기본 OFF로 바꾼다** — 앞으로는 아무 플래그
  없이 빌드하면 항상 키 없는(공개 가능한) 버전이 나오게 하고, 개인용(키 포함) 빌드만 명시적
  플래그로 만들게 함(안전한 기본값 원칙). (c) 저장소 커밋이 아니라 GitHub Release 첨부파일로
  올린다(히스토리 비대화 방지).
- 반영: `installer/lite/setup.iss` — `[Files]`의 `user_api_bundle\User_API\*` 항목을
  `#ifdef BUNDLE_API_KEY` ... `#endif`로 감쌈. 개인용 빌드는 `ISCC /DBUNDLE_API_KEY setup.iss`로
  명시적으로 켜야 함. `installer/lite/README.md` — "개인용으로 빌드하기"/"공개용(키 없음)으로
  빌드하기" 두 절차로 분리해서 재작성. 플래그 없이 컴파일해서 `데이터시트다운로더Lite_설치_
  공개용.exe` 생성 → GitHub Release `lite-v1.1.0`에 `DatasheetDownloaderLite_Setup.exe`로 첨부
  (한글/공백이 섞인 로컬 파일명을 그대로 업로드하면 자산 파일명이 깨지는 걸 발견해서, ASCII
  파일명으로 복사한 뒤 업로드하는 방식으로 우회함).
  **실제 사고(업로드 전에 잡아냄)**: 첫 컴파일 때 `installer/lite/setup.iss`는 방금 고쳤지만
  **빌드용 스테이징 폴더(임시 캐시)의 사본을 다시 복사해 넣는 걸 깜빡해서**, 옛 버전(항상 키
  포함)으로 컴파일돼 실제 키가 든 파일이 한 번 나옴. 업로드 직전 검증 단계(설치 후 `User_API`
  폴더 존재 여부 + 설치된 모든 텍스트 파일을 API 키 모양(GUID 정규식)으로 전수 검색)에서
  발견해서 즉시 삭제, 스테이징 재동기화 후 재빌드 → 같은 검증을 다시 통과한 것만 업로드함.
  GitHub에는 키가 든 버전이 올라간 적 없음. 검증: 설치 후 `User_API` 폴더 없음 + GUID 패턴
  전수 검색 0건 확인, `curl`로 실제 공개 다운로드 성공(HTTP 200, 크기·SHA-256 해시 일치)까지
  확인. **이 사고는 "Lite 배포판 빌드 시 반드시 원본과 재동기화 여부부터 확인할 것"(2026-09-09
  결정)이 정확히 경고했던 상황이 실전에서 그대로 재발한 사례** - 앞으로 `installer/lite/` 파일을
  고친 뒤에는 스테이징 폴더 동기화 여부를 매번 직접 diff로 확인할 것.
